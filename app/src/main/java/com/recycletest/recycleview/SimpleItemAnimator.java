...
/**
 * A wrapper class for ItemAnimator that records View bounds and decides whether it should
 * run move, change, add or remove animations. This class also replicates the original
 * ItemAnimator API.
 * <p>
 * It uses {@link ItemHolderInfo} to track the bounds information of the Views. If you
 * would like to extend this class, you can override {@link #obtainHolderInfo()} method to
 * provide your own info class that extends {@link ItemHolderInfo}.
 */
public abstract class SimpleItemAnimator extends RecyclerView.ItemAnimator {
     ...
     * @param supportsChangeAnimations true if change animations are supported by this
     * ItemAnimator, false otherwise. If the property is false, the ItemAnimator will not
     * receive a call to {@link #animateChange(RecyclerView.ViewHolder, RecyclerView.ViewHolder, int, int, int, *
    ...
    public boolean animateDisappearance(@NonNull RecyclerView.ViewHolder viewHolder, @NonNull ItemHolderInfo preLayoutInfo, @Nullable ItemHolderInfo postLayoutInfo) {
        ...
        if (!viewHolder.isRemoved() && (oldLeft != newLeft || oldTop != newTop)) {
            disappearingItemView.layout(newLeft, newTop, newLeft + disappearingItemView.getWidth(), newTop + disappearingItemView
                    .getHeight());
            if (DEBUG) {
     ...
     * whether and how to animate that change, but must always call {@link
     * #dispatchRemoveFinished(RecyclerView.ViewHolder)} when done, either immediately (if no animation
     * will occur) or after the animation actually finishes. The return value indicates
     * whether an animation has been set up and whether the ItemAnimator's {@link
     * #runPendingAnimations()} method should be called at the next opportunity. This
     * mechanism allows ItemAnimator to set up individual animations as separate calls to
     * {@link #animateAdd(RecyclerView.ViewHolder) animateAdd()}, {@link #animateMove(RecyclerView.ViewHolder, int,
     * * int, int, int) animateMove()}, {@link #animateRemove(RecyclerView.ViewHolder)
     * animateRemove()}, and {@link #animateChange(RecyclerView.ViewHolder, RecyclerView.ViewHolder, int, int, int,
     * * int)} come in one by one, then start the animations together in the later call to
     * {@link #runPendingAnimations()}. <p> <p>This method may also be called for
     * disappearing items which continue to exist in the RecyclerView, but for which the
     * system does not have enough information to animate them out of view. In that case,
     * the default animation for removing items is run on those items as well.</p>
     * @param holder The item that is being removed.
     * @return true if a later call to {@link #runPendingAnimations()} is requested, false
     * otherwise.
     */
    public abstract boolean animateRemove(RecyclerView.ViewHolder holder);

    /**
     * Called when an item is added to the RecyclerView. Implementors can choose whether
     * and how to animate that change, but must always call {@link
     * #dispatchAddFinished(RecyclerView.ViewHolder)} when done, either immediately (if no animation
     * will occur) or after the animation actually finishes. The return value indicates
     * whether an animation has been set up and whether the ItemAnimator's {@link
     * #runPendingAnimations()} method should be called at the next opportunity. This
     * mechanism allows ItemAnimator to set up individual animations as separate calls to
     ...
     * * int)} come in one by one, then start the animations together in the later call to
     * {@link #runPendingAnimations()}. <p> <p>This method may also be called for
     * appearing items which were already in the RecyclerView, but for which the system
     * does not have enough information to animate them into view. In that case, the
     * default animation for adding items is run on those items as well.</p>
    ...
    /**
     * Called when an item is moved in the RecyclerView. Implementors can choose whether
     * and how to animate that change, but must always call {@link
     * #dispatchMoveFinished(RecyclerView.ViewHolder)} when done, either immediately (if no animation
     * will occur) or after the animation actually finishes. The return value indicates
     * whether an animation has been set up and whether the ItemAnimator's {@link
     * #runPendingAnimations()} method should be called at the next opportunity. This
     * mechanism allows ItemAnimator to set up individual animations as separate calls to
     ...
     * {@link RecyclerView.Adapter#notifyItemChanged(int)} or {@link RecyclerView.Adapter#notifyItemRangeChanged(int,
     * * int)}.
     * <p>
     * Implementers can choose whether and how to animate changes, but must always call
     * {@link #dispatchChangeFinished(RecyclerView.ViewHolder, boolean)} for each non-null distinct
     * ViewHolder, either immediately (if no animation will occur) or after the animation
     * actually finishes. If the {@code oldHolder} is the same ViewHolder as the {@code
     * newHolder}, you must call {@link #dispatchChangeFinished(RecyclerView.ViewHolder, boolean)} once
     * and only once. In that case, the second parameter of {@code dispatchChangeFinished}
     * is ignored.
     * <p>
     * The return value indicates whether an animation has been set up and whether the
     * ItemAnimator's {@link #runPendingAnimations()} method should be called at the next
     * opportunity. This mechanism allows ItemAnimator to set up individual animations as
     * separate calls to {@link #animateAdd(RecyclerView.ViewHolder) animateAdd()}, {@link
     * #animateMove(RecyclerView.ViewHolder, int, int, int, int) animateMove()}, {@link
     * #animateRemove(RecyclerView.ViewHolder) animateRemove()}, and {@link #animateChange(RecyclerView.ViewHolder,
     * * RecyclerView.ViewHolder , int, int, int, int)} come in one by one, then start the animations
     * together in the later call to {@link #runPendingAnimations()}.
     * @param oldHolder The original item that changed.
     * @param newHolder The new item that was created with the changed content. Might be
     * null
     ...
     * @see RecyclerView.ItemAnimator#animateDisappearance(RecyclerView.ViewHolder, ItemHolderInfo, *
     * ItemHolderInfo)
     * @see RecyclerView.ItemAnimator#animatePersistence(RecyclerView.ViewHolder, ItemHolderInfo, *
     * ItemHolderInfo)
     * @see RecyclerView.ItemAnimator#animateAppearance(RecyclerView.ViewHolder, ItemHolderInfo, *
     * ItemHolderInfo)
     */
     ...
     * @param item The item which has been changed (this method must be called for each
     * non-null ViewHolder passed into {@link #animateChange(RecyclerView.ViewHolder, RecyclerView.ViewHolder, int,
     * * int, int, int)}).
     * @param oldItem true if this is the old item that was changed, false if it is the
     * new item that replaced the old item.
