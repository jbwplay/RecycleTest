 ...
 * <p>
 * To support animations, RecyclerView presents an older version the Adapter to best
 * represent previous state of the layout. Sometimes, this is not trivial when items are
 * removed that were not laid out, in which case, RecyclerView has no way of providing
 * that item's view for animations.
 * <p>
 * AdapterHelper creates an UpdateOp for each adapter data change then pre-processes them.
 * During pre processing, AdapterHelper finds out which UpdateOps can be deferred to
 * second layout pass and which cannot. For the UpdateOps that cannot be deferred,
 * AdapterHelper will change them according to previously deferred operation and dispatch
 * them before the first layout pass. It also takes care of updating deferred UpdateOps
 * since order of operations is changed by this process.
 * <p>
 * Although operations may be forwarded to LayoutManager in different orders, resulting
 * data set is guaranteed to be the consistent.
 */
