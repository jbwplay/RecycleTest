...
import com.recycletest.recycleview.helper.ItemTouchHelper;
...
public class LinearLayoutManager extends RecyclerView.LayoutManager implements ItemTouchHelper.ViewDropHandler, RecyclerView.SmoothScroller.ScrollVectorProvider {
     ...
     * than this factor times the total space of the list. If layout is vertical, total
     * space is the height minus padding, if layout is horizontal, total space is the
     * width minus padding.
     */
    ...
    /**
     * Helper class that keeps temporary layout state. It does not keep state after layout
     * is complete but we still keep a reference to re-use the same object.
     */
    private LayoutState mLayoutState;

    /**
     * Many calculations are made depending on orientation. To keep it clean, this
     * interface helps {@link LinearLayoutManager} make those decisions. Based on {@link
     * #mOrientation}, an implementation is lazily created in {@link #ensureLayoutState}
     * method.
     */
    ...
    /**
     * This keeps the final value for how LayoutManager should start laying out views. It
     * is calculated by checking {@link #getReverseLayout()} and View's layout direction.
     * {@link #onLayoutChildren(RecyclerView.Recycler, RecyclerView.State)} is run.
     */
    boolean mShouldReverseLayout = false;

    /**
     * Works the same way as {@link android.widget.AbsListView#setStackFromBottom(boolean)}
     * and it supports both orientations. see {@link android.widget.AbsListView#setStackFromBottom(boolean)}
     */
    ...
    /**
     * Re-used variable to keep anchor information on re-layout. Anchor position and
     * coordinate defines the reference point for LLM while doing a layout.
     */
     ...
     * RecyclerView.
     * <p>
     * If you are using a {@link RecyclerView.RecycledViewPool}, it might be a good idea
     * to set this flag to <code>true</code> so that views will be available to other
     * RecyclerViews immediately.
     * <p>
     * Note that, setting this flag will result in a performance drop if RecyclerView is
     * restored.
     * @param recycleChildrenOnDetach Whether children should be recycled in detach or
     * not.
     */
    ...
    public Parcelable onSaveInstanceState() {
        ...
        if (getChildCount() > 0) {
            ...
            if (didLayoutFromEnd) {
                ...
                state.mAnchorOffset = mOrientationHelper.getEndAfterPadding() - mOrientationHelper
                        .getDecoratedEnd(refChild);
                state.mAnchorPosition = getPosition(refChild);
            } else {
                final View refChild = getChildClosestToStart();
                state.mAnchorPosition = getPosition(refChild);
                state.mAnchorOffset = mOrientationHelper.getDecoratedStart(refChild) - mOrientationHelper
    ...
    /**
     * Calculates the view layout order. (e.g. from end to start or start to end) RTL
     * layout support is applied automatically. So if layout is RTL and {@link
     * #getReverseLayout()} is {@code true}, elements will be laid out starting from
     * left.
     */
    ...
    /**
     * Used to reverse item traversal and layout order. This behaves similar to the layout
     * change for RTL views. When set to true, first item is laid out at the end of the
     * UI, second item is laid out before it etc.
     * <p>
     * For horizontal layouts, it depends on the layout direction. When set to true, If
     * {@link RecyclerView} is LTR, than it will layout from RTL, if {@link RecyclerView}}
     * is RTL, it will layout from LTR.
     * <p>
     * If you are looking for the exact same behavior of {@link
     * android.widget.AbsListView#setStackFromBottom(boolean)}, use {@link
     ...
     * <p>
     * <p>By default, {@link LinearLayoutManager} lays out 1 extra page of items while
     * smooth scrolling and 0 otherwise. You can override this method to implement your
     * custom layout pre-cache logic.</p>
     * <p>
     * <p><strong>Note:</strong>Laying out invisible elements generally comes with
     * significant performance cost. It's typically only desirable in places like smooth
     * scrolling to an unknown location, where 1) the extra content helps
     * LinearLayoutManager know in advance when its target is approaching, so it can
     * decelerate early and smoothly and 2) while motion is continuous.</p>
     * <p>
     * <p>Extending the extra layout space is especially expensive if done while the user
     * may change scrolling direction. Changing direction will cause the extra layout
     * space to swap to the opposite side of the viewport, incurring many
     * rebinds/recycles, unless the cache is large enough to handle it.</p>
    ...
    public void onLayoutChildren(RecyclerView.Recycler recycler, RecyclerView.State state) {
        ...
        } else if (focused != null && (mOrientationHelper.getDecoratedStart(focused) >= mOrientationHelper
        ...
        if (state.isPreLayout() && mPendingScrollPosition != RecyclerView.NO_POSITION && mPendingScrollPositionOffset != INVALID_OFFSET) {
            ...
            if (existing != null) {
                ...
                if (mShouldReverseLayout) {
                    current = mOrientationHelper.getEndAfterPadding() - mOrientationHelper
                            .getDecoratedEnd(existing);
                    upcomingOffset = current - mPendingScrollPositionOffset;
                } else {
                    current = mOrientationHelper.getDecoratedStart(existing) - mOrientationHelper
                            .getStartAfterPadding();
     ...
     * accordingly or even update anchor info if necessary.
     ...
     * @param firstLayoutItemDirection The direction of the first layout filling in terms
     * of adapter indices.
     */
     ...
     * closest to start or end that has a valid position (e.g. not removed).
     * <p>
        ...
        if (referenceChild != null) {
            ...
            if (!state.isPreLayout() && supportsPredictiveItemAnimations()) {
                ...
                final boolean notVisible = mOrientationHelper.getDecoratedStart(referenceChild) >= mOrientationHelper
                        ...
                        .getStartAfterPadding();
                if (notVisible) {
                    anchorInfo.mCoordinate = anchorInfo.mLayoutFromEnd ? mOrientationHelper
    ...
    /**
     * If there is a pending scroll position or saved states, updates the anchor info from
     * that data and returns true
     */
    private boolean updateAnchorFromPendingData(RecyclerView.State state, AnchorInfo anchorInfo) {
        ...
        if (mPendingScrollPositionOffset == INVALID_OFFSET) {
            ...
            if (child != null) {
                ...
                final int startGap = mOrientationHelper.getDecoratedStart(child) - mOrientationHelper
                        .getStartAfterPadding();
                if (startGap < 0) {
                ...
                final int endGap = mOrientationHelper.getEndAfterPadding() - mOrientationHelper
                ...
                anchorInfo.mCoordinate = anchorInfo.mLayoutFromEnd ? (mOrientationHelper.getDecoratedEnd(child) + mOrientationHelper
                        .getTotalSpaceChange()) : mOrientationHelper.getDecoratedStart(child);
    ...
    /**
     * <p>Scroll the RecyclerView to make the position visible.</p> <p> <p>RecyclerView
     * will scroll the minimum amount that is necessary to make the target position
     * visible. If you are looking for a similar behavior to {@link
     * android.widget.ListView#setSelection(int)} or {@link android.widget.ListView#setSelectionFromTop(int,
     * * int)}, use {@link #scrollToPositionWithOffset(int, int)}.</p> <p> <p>Note that
     * scroll position change will not be reflected until the next layout call.</p>
     ...
     * start. Resolved layout start depends on {@link #getReverseLayout()}, {@link
     * ViewCompat#getLayoutDirection(View)} and {@link #getStackFromEnd()}.
     * <p>
     * For example, if layout is {@link #VERTICAL} and {@link #getStackFromEnd()} is true,
     * calling <code>scrollToPositionWithOffset(10, 20)</code> will layout such that
     * <code>item[10]</code>'s bottom is 20 pixels above the RecyclerView's bottom.
     * <p>
     * Note that scroll position change will not be reflected until the next layout call.
     * <p>
     * If you are just trying to make a position visible, use {@link
     ...
     * computed based on the number of visible pixels in the visible items. This however
     * assumes that all list items have similar or equal widths or heights (depending on
     * list orientation). If you use a list in which items have different dimensions, the
     * scrollbar will change appearance as the user scrolls through the list. To avoid
     * this issue,  you need to disable this property.
     * <p>
     * When smooth scrollbar is disabled, the position and size of the scrollbar thumb is
     * based solely on the number of items in the adapter and the position of the visible
     * items inside the adapter. This provides a stable scrollbar as the user navigates
     * through a list of items with varying widths / heights.
    ...
    private void updateLayoutState(int layoutDirection, int requiredSpace, boolean canUseExistingSpace, RecyclerView.State state) {
        ...
        if (layoutDirection == LayoutState.LAYOUT_END) {
            ...
            scrollingOffset = mOrientationHelper.getDecoratedEnd(child) - mOrientationHelper
                    .getEndAfterPadding();
            ...
            scrollingOffset = -mOrientationHelper.getDecoratedStart(child) + mOrientationHelper
                    .getStartAfterPadding();
        }
    ...
    boolean resolveIsInfinite() {
        return mOrientationHelper.getMode() == View.MeasureSpec.UNSPECIFIED && mOrientationHelper
                .getEnd() == 0;
    }
    ...
    /**
     * Sets the number of items to prefetch in {@link #collectInitialPrefetchPositions(int,
     * * LayoutPrefetchRegistry)}, which defines how many inner items should be prefetched
     * when this LayoutManager's RecyclerView is nested inside another RecyclerView. <p>
     * <p>Set this value to the number of items this inner LayoutManager will display when
     * it is first scrolled into the viewport. RecyclerView will attempt to prefetch that
     * number of items so they are ready, avoiding jank as the inner RecyclerView is
     * scrolled into the viewport.</p> <p> <p>For example, take a vertically scrolling
     * RecyclerView with horizontally scrolling inner RecyclerViews. The rows always have
     * 4 items visible in them (or 5 if not aligned). Passing <code>4</code> to this
     * method for each inner RecyclerView's LinearLayoutManager will enable RecyclerView's
     * prefetching feature to do create/bind work for 4 views within a row early, before
     * it is scrolled on screen, instead of just the default 2.</p> <p> <p>Calling this
     * method does nothing unless the LayoutManager is in a RecyclerView nested in another
     * RecyclerView.</p> <p> <p class="note"><strong>Note:</strong> Setting this value to
     * be larger than the number of views that will be visible in this view can incur
     * unnecessary bind work, and an increase to the number of Views created and in active
     * use.</p>
    ...
    /**
     * Gets the number of items to prefetch in {@link #collectInitialPrefetchPositions(int,
     * * LayoutPrefetchRegistry)}, which defines how many inner items should be prefetched
     * when this LayoutManager's RecyclerView is nested inside another RecyclerView.
     ...
     * layout.
     * <p>
     * Checks both layout position and visible position to guarantee that the view is not
     * visible.
     * @param recycler Recycler instance of {@link RecyclerView}
     * @param dt This can be used to add additional padding to the visible area. This is
     * used to detect children that will go out of bounds after scrolling, without
    ...
    private void recycleViewsFromStart(RecyclerView.Recycler recycler, int dt) {
        ...
        if (mShouldReverseLayout) {
            for (int i = childCount - 1; i >= 0; i--) {
                ...
                if (mOrientationHelper.getDecoratedEnd(child) > limit || mOrientationHelper
    ...
    /**
     * Recycles views that went out of bounds after scrolling towards the start of the
     * layout.
     * <p>
     * Checks both layout position and visible position to guarantee that the view is not
     * visible.
        ...
        if (mShouldReverseLayout) {
            for (int i = 0; i < childCount; i++) {
                ...
                if (mOrientationHelper.getDecoratedStart(child) < limit || mOrientationHelper
                        .getTransformedStartWithDecoration(child) < limit) {
                    // stop here
                    recycleChildren(recycler, 0, i);
                    return;
                }
     ...
     * we may consider moving it out of this view so passing around as a parameter for
     * now, rather than accessing {@link #mLayoutState}
    ...
    /**
     * The magic functions :). Fills the given layout, defined by the layoutState. This is
     * fairly independent from the rest of the {@link LinearLayoutManager} and with little
    ...
    int fill(RecyclerView.Recycler recycler, LayoutState layoutState, RecyclerView.State state, boolean stopOnFocusable) {
        ...
        while ((layoutState.mInfinite || remainingSpace > 0) && layoutState.hasMore(state)) {
             ...
             */
            if (!layoutChunkResult.mIgnoreConsumed || mLayoutState.mScrapList != null || !state
                    .isPreLayout()) {
     ...
     * @return {@link LayoutState#LAYOUT_START} or {@link LayoutState#LAYOUT_END} if focus
     * direction is applicable to current state, {@link LayoutState#INVALID_LAYOUT}
     * otherwise.
     */
    ...
    /**
     * Convenience method to find the child closes to start. Caller should check it has
     * enough children.
    ...
    /**
     * Convenience method to find the child closes to end. Caller should check it has
     * enough children.
    ...
    /**
     * Convenience method to find the visible child closes to start. Caller should check
     * if it has enough children.
    ...
    /**
     * Convenience method to find the visible child closes to end. Caller should check if
    ...
    /**
     * Among the children that are suitable to be considered as an anchor child, returns
     * the one closest to the end of the layout.
     * <p>
     * Due to ambiguous adapter updates or children being removed, some children's
     * positions may be invalid. This method is a best effort to find a position within
     * adapter bounds if possible.
     * <p>
    ...
    /**
     * Among the children that are suitable to be considered as an anchor child, returns
     * the one closest to the start of the layout.
     * <p>
     * Due to ambiguous adapter updates or children being removed, some children's
     * positions may be invalid. This method is a best effort to find a position within
     * adapter bounds if possible.
    ...
    View findReferenceChild(RecyclerView.Recycler recycler, RecyclerView.State state, int start, int end, int itemCount) {
        ...
        for (int i = start; i != end; i += diff) {
            ...
            if (position >= 0 && position < itemCount) {
                ...
                } else if (mOrientationHelper.getDecoratedStart(view) >= boundsEnd || mOrientationHelper
                        .getDecoratedEnd(view) < boundsStart) {
    ...
    private View findPartiallyOrCompletelyInvisibleChildClosestToEnd(RecyclerView.Recycler recycler, RecyclerView.State state) {
        return mShouldReverseLayout ? findFirstPartiallyOrCompletelyInvisibleChild(recycler, state) : findLastPartiallyOrCompletelyInvisibleChild(recycler, state);
    }
    ...
    private View findPartiallyOrCompletelyInvisibleChildClosestToStart(RecyclerView.Recycler recycler, RecyclerView.State state) {
        return mShouldReverseLayout ? findLastPartiallyOrCompletelyInvisibleChild(recycler, state) : findFirstPartiallyOrCompletelyInvisibleChild(recycler, state);
    }
    ...
    /**
     * Returns the adapter position of the first visible view. This position does not
     * include adapter changes that were dispatched after the last layout pass.
     * <p>
     * Note that, this value is not affected by layout orientation or item order
     * traversal. ({@link #setReverseLayout(boolean)}). Views are sorted by their
     * positions in the adapter, not in the layout.
     * <p>
     * If RecyclerView has item decorators, they will be considered in calculations as
     * well.
     * <p>
     * LayoutManager may pre-cache some views that are not necessarily visible. Those
     * views are ignored in this method.
     * @return The adapter position of the first visible item or {@link
    ...
    /**
     * Returns the adapter position of the first fully visible view. This position does
     * not include adapter changes that were dispatched after the last layout pass.
     * <p>
     ...
     * LayoutManager may pre-cache some views that are not necessarily visible. Those
     * views are ignored in this method.
     * @return The adapter position of the last visible view or {@link
     * RecyclerView#NO_POSITION} if there aren't any visible items.
     * @see #findLastCompletelyVisibleItemPosition()
     * @see #findFirstVisibleItemPosition()
     */
    ...
    /**
     * Returns the adapter position of the last fully visible view. This position does not
     * include adapter changes that were dispatched after the last layout pass.
     * <p>
     * Note that bounds check is only performed in the current orientation. That means, if
     * LayoutManager is horizontal, it will only check the view's left and right edges.
    ...
    View findOneVisibleChild(int fromIndex, int toIndex, boolean completelyVisible, boolean acceptPartiallyVisible) {
        ...
        return (mOrientation == HORIZONTAL) ? mHorizontalBoundCheck.findOneViewWithinBoundFlags(fromIndex, toIndex, preferredBoundsFlag, acceptableBoundsFlag) : mVerticalBoundCheck
                .findOneViewWithinBoundFlags(fromIndex, toIndex, preferredBoundsFlag, acceptableBoundsFlag);
    }
    ...
    View findOnePartiallyOrCompletelyInvisibleChild(int fromIndex, int toIndex) {
        ...
        @ViewBoundsCheck.ViewBounds
        int acceptableBoundsFlag = 0;
        if (mOrientationHelper.getDecoratedStart(getChildAt(fromIndex)) < mOrientationHelper
                .getStartAfterPadding()) {
    ...
    /**
     * Used for debugging. Validates that child views are laid out in correct order. This
     * is important because rest of the algorithm relies on this constraint.
     * <p>
     * In default layout, child 0 should be closest to screen position 0 and last child
     * should be closest to position WIDTH or HEIGHT. In reverse layout, last child should
     * be closes to screen position 0 and first child should be closest to position WIDTH
     * or HEIGHT
     */
    ...
    public void prepareForDrop(View view, View target, int x, int y) {
        ...
        if (mShouldReverseLayout) {
            if (dropDirection == LayoutState.ITEM_DIRECTION_TAIL) {
                scrollToPositionWithOffset(targetPos, mOrientationHelper.getEndAfterPadding() - (mOrientationHelper
                        .getDecoratedStart(target) + mOrientationHelper.getDecoratedMeasurement(view)));
            } else {
                scrollToPositionWithOffset(targetPos, mOrientationHelper.getEndAfterPadding() - mOrientationHelper
            ...
            if (dropDirection == LayoutState.ITEM_DIRECTION_HEAD) {
                ...
                scrollToPositionWithOffset(targetPos, mOrientationHelper.getDecoratedEnd(target) - mOrientationHelper
    ...
    /**
     * Helper class that keeps temporary state while {LayoutManager} is filling out the
     * empty space.
     */
    static class LayoutState {
        ...
        /**
         * Used when LayoutState is constructed in a scrolling state. It should be set the
         * amount of scrolling we can make without creating a new view. Settings this is
         * required for efficient view recycling.
         */
        int mScrollingOffset;

        /**
         * Used if you want to pre-layout items that are not yet visible. The difference
         * with {@link #mAvailable} is that, when recycling, distance laid out for {@link
         ...
         * this value is set to true, we skip removed views since they should not be laid
         * out in post layout step.
         */
        boolean mIsPreLayout = false;

        /**
         * The most recent {@link #scrollBy(int, RecyclerView.Recycler, *
         * RecyclerView.State)} amount.
         */
        int mLastScrollDelta;

        /**
         * When LLM needs to layout particular views, it sets this list in which case,
         * LayoutState will only return views from this list and return null if it cannot
         * find an item.
         */
        ...
        /**
         * Gets the view for the next element that we should layout. Also updates current
         * item index to the next item, based on {@link #mItemDirection}
    ...
    class AnchorInfo {
        ...
        void assignCoordinateFromPadding() {
            mCoordinate = mLayoutFromEnd ? mOrientationHelper.getEndAfterPadding() : mOrientationHelper
                    .getStartAfterPadding();
        }
        ...
        boolean isViewValidAsAnchor(View child, RecyclerView.State state) {
            ...
            return !lp.isItemRemoved() && lp.getViewLayoutPosition() >= 0 && lp.getViewLayoutPosition() < state
                    .getItemCount();
        }
        ...
        public void assignFromViewAndKeepVisibleRect(View child) {
            ...
            if (mLayoutFromEnd) {
                ...
                if (startMargin > 0) { // we have room to fix end as well
                    ...
                    final int endReference = mOrientationHelper.getEndAfterPadding() - Math
                            .min(0, previousEndMargin);
        ...
        public void assignFromView(View child) {
            if (mLayoutFromEnd) {
                mCoordinate = mOrientationHelper.getDecoratedEnd(child) + mOrientationHelper
                        .getTotalSpaceChange();
            } else {
