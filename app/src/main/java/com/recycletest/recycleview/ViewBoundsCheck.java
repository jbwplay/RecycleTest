...
/**
 * A utility class used to check the boundaries of a given view within its parent view
 * based on a set of boundary flags.
 */
class ViewBoundsCheck {
    ...
    /**
     * The child view's start can be equal to its parent view's start. This flag follows
     * with GT or LT indicating greater (less) than or equal relation.
     */
    ...
    /**
     * The child view's start can be equal to its parent view's end. This flag follows
     * with GT or LT indicating greater (less) than or equal relation.
     */
    ...
    /**
     * The child view's end can be equal to its parent view's start. This flag follows
     * with GT or LT indicating greater (less) than or equal relation.
     */
    ...
    /**
     * The child view's end can be equal to its parent view's end. This flag follows with
     * GT or LT indicating greater (less) than or equal relation.
     */
    ...
    /**
     * The set of flags that can be passed for checking the view boundary conditions. CVS
     * in the flag name indicates the child view, and PV indicates the parent view.\ The
     * following S, E indicate a view's start and end points, respectively. GT and LT
     * indicate a strictly greater and less than relationship. Greater than or equal (or
     * less than or equal) can be specified by setting both GT and EQ (or LT and EQ)
     * flags. For instance, setting both {@link #FLAG_CVS_GT_PVS} and {@link
     * #FLAG_CVS_EQ_PVS} indicate the child view's start should be greater than or equal
     * to its parent start.
     */
    ...
    /**
     * Returns the first view starting from fromIndex to toIndex in views whose bounds lie
     * within its parent bounds based on the provided preferredBoundFlags. If no match is
     * found based on the preferred flags, and a nonzero acceptableBoundFlags is
     * specified, the last view whose bounds lie within its parent view based on the
     * acceptableBoundFlags is returned. If no such view is found based on either of these
     * two flags, null is returned.
     ...
     * @param preferredBoundFlags The flags indicating the preferred match. Once a match
     * is found based on this flag, that view is returned instantly.
     * @param acceptableBoundFlags The flags indicating the acceptable match if no
     * preferred match is found. If so, and if acceptableBoundFlags is non-zero, the last
     * matching acceptable view is returned. Otherwise, null is returned.
     * @return The first view that satisfies acceptableBoundFlags or the last view
     * satisfying acceptableBoundFlags boundary conditions.
     */
    ...
    /**
     * Returns whether the specified view lies within the boundary condition of its parent
     * view.
     * @param child The child view to be checked.
     * @param boundsFlags The flag against which the child view and parent view are
     * matched.
    ...
    boolean isViewWithinBoundFlags(View child, @ViewBounds int boundsFlags) {
        mBoundFlags.setBounds(mCallback.getParentStart(), mCallback.getParentEnd(), mCallback
                .getChildStart(child), mCallback.getChildEnd(child));
        if (boundsFlags != 0) {
            mBoundFlags.resetFlags();
    ...
    /**
     * Callback provided by the user of this class in order to retrieve information about
