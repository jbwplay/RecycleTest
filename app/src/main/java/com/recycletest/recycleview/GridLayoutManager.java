...
public class GridLayoutManager extends LinearLayoutManager {
    ...
    public void onInitializeAccessibilityNodeInfoForItem(RecyclerView.Recycler recycler, RecyclerView.State state, View host, AccessibilityNodeInfoCompat info) {
        ...
        if (mOrientation == HORIZONTAL) {
            info.setCollectionItemInfo(AccessibilityNodeInfoCompat.CollectionItemInfoCompat
                    .obtain(glp.getSpanIndex(), glp.getSpanSize(), spanGroupIndex, 1, mSpanCount > 1 && glp
                            .getSpanSize() == mSpanCount, false));
        } else { // VERTICAL
            info.setCollectionItemInfo(AccessibilityNodeInfoCompat.CollectionItemInfoCompat
                    .obtain(spanGroupIndex, 1, glp.getSpanIndex(), glp.getSpanSize(), mSpanCount > 1 && glp
                            .getSpanSize() == mSpanCount, false));
        }
    }
     ...
     * @param spanSizeLookup {@link SpanSizeLookup} instance to be used to query number of
     * spans occupied by each item
     */
     ...
     * @param totalSpace total available space after padding is removed
     * @return The updated array. Might be the same instance as the provided array if its
    ...
    /**
     * Measures a child with currently known information. This is not necessarily the
     * child's final measurement. (see fillChunk for details).
     * @param view The child view to be measured
     * @param otherDirParentSpecMode The RV measure spec that should be used in the
    ...
    /**
     * This is called after laying out a row (if vertical) or a column (if horizontal)
     * when the RecyclerView does not have exact measurement specs.
     * <p>
     * Here we try to assign a best guess width or height and re-do the layout to update
     * other views that wanted to MATCH_PARENT in the non-scroll orientation.
     * @param maxSizeInOther The maximum size per span ratio from the measurement of the
     * children.
     * @param currentOtherDirSize The size before this layout chunk. There is no reason to
     * go below.
     */
    ...
    public abstract static class SpanSizeLookup {
        ...
        /**
         * Sets whether the results of {@link #getSpanIndex(int, int)} method should be
         * cached or not. By default these values are not cached. If you are not
         * overriding {@link #getSpanIndex(int, int)}, you should set this to true for
         * better performance.
         * @param cacheSpanIndices Whether results of getSpanIndex should be cached or
         * not.
         */
        ...
        /**
         * Clears the span index cache. GridLayoutManager automatically calls this method
        ...
        /**
         * Returns whether results of {@link #getSpanIndex(int, int)} method are cached or
         * not.
         ...
         * <p>
         * If you have a faster way to calculate span index for your items, you should
         * override this method. Otherwise, you should enable span index cache ({@link
         * #setSpanIndexCacheEnabled(boolean)}) for better performance. When caching is
         * disabled, default implementation traverses all items from 0 to
         * <code>position</code>. When caching is enabled, it calculates from the closest
         * cached value before the <code>position</code>.
         * <p>
         * If you override this method, you need to make sure it is consistent with {@link
         * #getSpanSize(int)}. GridLayoutManager does not call this method for each item.
         * It is called only for the reference item and rest of the items are assigned to
         * spans based on the reference item. For example, you cannot assign a position to
         * span 2 while span 1 is empty.
         * <p>
         * Note that span offsets always start with 0 and are not affected by RTL.
         * @param position The position of the item
         * @param spanCount The total number of spans in the grid
         * @return The final span position of the item. Should be between 0 (inclusive)
         * and <code>spanCount</code>(exclusive)
         */
         ...
         * <p>
         * For example, if grid has 3 columns and each item occupies 1 span, span group
         * index for item 1 will be 0, item 5 will be 1.
         * @param adapterPosition The position in adapter
         * @param spanCount The total number of spans in the grid
         * @return The index of the span group including the item at the given adapter
         * position
         */
    ...
    public View onFocusSearchFailed(View focused, int focusDirection, RecyclerView.Recycler recycler, RecyclerView.State state) {
        ...
        for (int i = start; i != limit; i += inc) {
            ...
            if ((candidate.hasFocusable() && focusableWeakCandidate == null) || (!candidate
                    .hasFocusable() && unfocusableWeakCandidate == null)) {
                assignAsWeek = true;
            } else {
            ...
            if (assignAsWeek) {
                if (candidate.hasFocusable()) {
                    ...
                    focusableWeakCandidateOverlap = Math.min(candidateEnd, prevSpanEnd) - Math
                    ...
                    unfocusableWeakCandidateOverlap = Math.min(candidateEnd, prevSpanEnd) - Math
                            .max(candidateStart, prevSpanStart);
                }
            }
        }
    ...
    public static class LayoutParams extends RecyclerView.LayoutParams {
        ...
        /**
         * Returns the current span index of this View. If the View is not laid out yet,
         * the return value is <code>undefined</code>.
         * <p>
         * Starting with RecyclerView <b>24.2.0</b>, span indices are always indexed from
         * position 0 even if the layout is RTL. In a vertical GridLayoutManager,
         * <b>leftmost</b> span is span 0 if the layout is <b>LTR</b> and <b>rightmost</b>
         * span is span 0 if the layout is <b>RTL</b>. Prior to 24.2.0, it was the
         * opposite which was conflicting with {@link SpanSizeLookup#getSpanIndex(int, *
         * int)}.
         * <p>
        ...
        /**
         * Returns the number of spans occupied by this View. If the View not laid out
         * yet, the return value is <code>undefined</code>.
