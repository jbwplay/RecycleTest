...
package com.recycletest.recycleview.util;
...
/**
 * A Sorted list implementation that can keep items in order and also notify for changes
 * in the list such that it can be bound to a {@link RecyclerView.Adapter
 * RecyclerView.Adapter}.
 * <p>
 * It keeps items ordered using the {@link Callback#compare(Object, Object)} method and
 * uses binary search to retrieve items. If the sorting criteria of your items may change,
 * make sure you call appropriate methods while editing them to avoid data
 * inconsistencies.
 * <p>
 * You can control the order of items and change notifications via the {@link Callback}
 * parameter.
 */
...
public class SortedList<T> {
    ...
    /**
     * The callback instance that controls the behavior of the SortedList and get notified
     * when changes happen.
     */
     ...
     * <p>
     * If the item already exists in the list and its sorting criteria is not changed, it
     * is replaced with the existing Item. SortedList uses {@link
     * Callback#areItemsTheSame(Object, Object)} to check if two items are the same item
     * and uses {@link Callback#areContentsTheSame(Object, Object)} to decide whether it
     * should call {@link Callback#onChanged(int, int)} or not. In both cases, it always
     * removes the reference to the old item and puts the new item into the backing array
     * even if {@link Callback#areContentsTheSame(Object, Object)} returns false.
     * <p>
     * If the sorting criteria of the item is changed, SortedList won't be able to find
     * its duplicate in the list which will result in having a duplicate of the Item in
     * the list. If you need to update sorting criteria of an item that already exists in
     * the list, use {@link #updateItemAt(int, Object)}. You can find the index of the
     * item using {@link #indexOf(Object)} before you update the object.
    ...
    /**
     * Adds the given items to the list. Equivalent to calling {@link SortedList#add} in a
     * loop, except the callback events may be in a different order/granularity since
     * addAll can batch them for better performance. <p> If allowed, may modify the input
     * array and even take the ownership over it in order to avoid extra memory allocation
     * during sorting and deduplication. </p>
     ...
     * {@link #endBatchedUpdates()}. For example, if you add multiple items in a loop and
     * they are placed into consecutive indices, SortedList calls {@link
     * Callback#onInserted(int, int)} only once with the proper item count. If an event
     * cannot be merged with the previous event, the previous event is dispatched to the
     * callback instantly.
     * <p>
     ...
     * {@link BatchedCallback}. In that case, you must make sure that you are manually
     * calling {@link BatchedCallback#dispatchLastEvent()} right after you complete your
     * data changes. Failing to do so may create data inconsistencies with the Callback.
     * <p>
     * If the current Callback is an instance of {@link BatchedCallback}, calling this
    ...
    /**
     * Removes the provided item from the list and calls {@link Callback#onRemoved(int, *
     * int)}.
    ...
    /**
     * Updates the item at the given index and calls {@link Callback#onChanged(int, int)}
     * and/or {@link Callback#onMoved(int, int)} if necessary.
     * <p>
     * You can use this method if you need to change an existing Item such that its
     * position in the list may change.
     * <p>
     * If the new object is a different object (<code>get(index) != item</code>) and
     * {@link Callback#areContentsTheSame(Object, Object)} returns <code>true</code>,
     * SortedList avoids calling {@link Callback#onChanged(int, int)} otherwise it calls
    ...
    /**
     * This method can be used to recalculate the position of the item at the given index,
     * without triggering an {@link Callback#onChanged(int, int)} callback.
     * <p>
     * If you are editing objects in the list such that their position in the list may
     * change but you don't want to trigger an onChange animation, you can use this method
     * to re-position it. If the item changes position, SortedList will call {@link
     * Callback#onMoved(int, int)} without calling {@link Callback#onChanged(int, int)}.
     * <p>
     ...
     * mSortedList.indexOf(item) will not be able to find the item. This is why the code
     * above first gets the position before editing the item, edits it and informs the
     * SortedList that item should be repositioned.
     ...
     * @return The position of the provided item or {@link #INVALID_POSITION} if item is
     * not in the list.
     */
     ...
     * <p>
     * SortedList calls the callback methods on this class to notify changes about the
     * underlying data.
     */
    public static abstract class Callback<T2> implements Comparator<T2>, ListUpdateCallback {
        ...
        /**
         * Called by the SortedList when it wants to check whether two items have the same
         * data or not. SortedList uses this information to decide whether it should call
         * {@link #onChanged(int, int)} or not.
         * <p>
         * SortedList uses this method to check equality instead of {@link
         * Object#equals(Object)} so that you can change its behavior depending on your
         * UI.
         * <p>
         * For example, if you are using SortedList with a {@link
         * RecyclerView.Adapter RecyclerView.Adapter}, you
         * should return whether the items' visual representations are the same or not.
        ...
        /**
         * Called by the SortedList to decide whether two object represent the same Item
         * or not.
         * <p>
         * For example, if your items have unique ids, this method should check their
         * equality.
    ...
    /**
     * A callback implementation that can batch notify events dispatched by the
     * SortedList.
     * <p>
     * This class can be useful if you want to do multiple operations on a SortedList but
     * don't want to dispatch each event one by one, which may result in a performance
     * issue.
     * <p>
     * For example, if you are going to add multiple items to a SortedList,
     * BatchedCallback call convert individual <code>onInserted(index, 1)</code> calls
     * into one <code>onInserted(index, N)</code> if items are added into consecutive
     * indices. This change can help RecyclerView resolve changes much more easily.
     * <p>
     * If consecutive changes in the SortedList are not suitable for batching,
     * BatchingCallback dispatches them as soon as such case is detected. After your edits
     * on the SortedList is complete, you <b>must</b> always call {@link
     * BatchedCallback#dispatchLastEvent()} to flush all changes to the Callback.
     */
    public static class BatchedCallback<T2> extends Callback<T2> {
         ...
         * @param wrappedCallback The Callback which should received the data change
         * callbacks. Other method calls (e.g. {@link #compare(Object, Object)} from the
         ...
         * You <b>must</b> always call this method after you are done with editing the
         * SortedList.
         */
