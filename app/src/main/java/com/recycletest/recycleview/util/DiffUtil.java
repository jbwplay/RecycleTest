 ...
 * <p>
 * DiffUtil uses Eugene W. Myers's difference algorithm to calculate the minimal number of
 * updates to convert one list into another. Myers's algorithm does not handle items that
 * are moved so DiffUtil runs a second pass on the result to detect items that were
 * moved.
 * <p>
 * If the lists are large, this operation may take significant time so you are advised to
 * run this on a background thread, get the {@link DiffResult} then apply it on the
 ...
 * number of added and removed items. If your lists are already sorted by the same
 * constraint (e.g. a created timestamp for a list of posts), you can disable move
 * detection to improve performance.
 * <p>
 * The actual runtime of the algorithm significantly depends on the number of changes in
 * the list and the cost of your comparison methods. Below are some average run times for
 * reference: (The test list is composed of random UUID Strings and the tests are run on
 * Nexus 5X with M) <ul> <li>100 items and 10 modifications: avg: 0.39 ms, median: 0.35 ms
 * <li>100 items and 100 modifications: 3.82 ms, median: 3.75 ms <li>100 items and 100
 * modifications without moves: 2.09 ms, median: 2.06 ms <li>1000 items and 50
 * modifications: avg: 4.67 ms, median: 4.59 ms <li>1000 items and 50 modifications
 * without moves: avg: 3.59 ms, median: 3.50 ms <li>1000 items and 200 modifications:
 * 27.07 ms, median: 26.92 ms <li>1000 items and 200 modifications without moves: 13.54
 * ms, median: 13.36 ms </ul>
 * <p>
...
public class DiffUtil {
    ...
    /**
     * Calculates the list of update operations that can covert one list into the other
     * one.
     * @param cb The callback that acts as a gateway to the backing list data
     * @return A DiffResult that contains the information about the edit sequence to
     * convert the old list into the new list.
     */
     ...
     * (swap positions), you can disable move detection which takes <code>O(N^2)</code>
     * time where N is the number of added, moved, removed items.
     * @param cb The callback that acts as a gateway to the backing list data
     * @param detectMoves True if DiffUtil should try to detect moved items, false
     * otherwise.
     * @return A DiffResult that contains the information about the edit sequence to
     * convert the old list into the new list.
     */
    public static DiffResult calculateDiff(Callback cb, boolean detectMoves) {
        ...
        while (!stack.isEmpty()) {
            ...
            if (snake != null) {
                ...
                final Range left = rangePool.isEmpty() ? new Range() : rangePool.remove(rangePool
                        .size() - 1);
    ...
    public abstract static class Callback {
         ...
         * <p>
         * For example, if your items have unique ids, this method should check their id
         * equality.
         ...
         * changed.
         * <p>
         * DiffUtil uses this method to check equality instead of {@link
         * Object#equals(Object)} so that you can change its behavior depending on your
         * UI. For example, if you are using DiffUtil with a {@link
         * RecyclerView.Adapter RecyclerView.Adapter}, you
         * should return whether the items' visual representations are the same.
         * <p>
         * This method is called only if {@link #areItemsTheSame(int, int)} returns {@code
         * true} for these items.
         * @param oldItemPosition The position of the item in the old list
         * @param newItemPosition The position of the item in the new list which replaces
         * the oldItem
         ...
         * {@link #areContentsTheSame(int, int)} returns false for them, DiffUtil calls
         * this method to get a payload about the change.
         * <p>
         * For example, if you are using DiffUtil with {@link RecyclerView}, you can
         * return the particular field that changed in the item and your {@link
         * RecyclerView.ItemAnimator ItemAnimator} can use that
    ...
    /**
     * Snakes represent a match between two lists. It is optionally prefixed or postfixed
     * with an add or remove operation. See the Myers' paper for details.
     */
    static class Snake {
        /**
        ...
        /**
         * If true, this is a removal from the original list followed by {@code size}
         * matches. If false, this is an addition from the new list followed by {@code
         * size} matches.
         */
        boolean removal;
    ...
    /**
     * This class holds the information about the result of a {@link
     ...
     * or directly stream the results into a {@link RecyclerView.Adapter} via {@link
     * #dispatchUpdatesTo(RecyclerView.Adapter)}.
     */
    public static class DiffResult {
        /**
         * While reading the flags below, keep in mind that when multiple items move in a
         * list, Myers's may pick any of them as the anchor item and consider that one
         * NOT_CHANGED while picking others as additions and removals. This is completely
         * fine as we later detect all moves.
         * <p>
         * Below, when an item is mentioned to stay in the same "location", it means we
         * won't dispatch a move/add/remove for it, it DOES NOT mean the item is still in
         * the same position.
         */
        ...
        /**
         * We always add a Snake to 0/0 so that we can run loops from end to beginning and
         * be done when we run out of snakes.
         */
         ...
         * <p>
         * DiffUtil does this pre-processing so that if it is running on a big list, it
         * can be moved to background thread where most of the expensive stuff will be
         * calculated and kept in the statuses maps. DiffResult uses this pre-calculated
         * information while dispatching the updates (which is probably being called on
         * the main thread).
         */
         ...
         * <p>
         * For example, if you have an {@link RecyclerView.Adapter
         * Adapter} that is backed by a {@link List}, you can swap the list with the new
         * one then call this method to dispatch all updates to the RecyclerView.
         * <pre>
         *     List oldList = mAdapter.getData();
         *     DiffResult result = DiffUtil.calculateDiff(new MyCallback(oldList,
         * newList));
         *     mAdapter.setData(newList);
         *     result.dispatchUpdatesTo(mAdapter);
         * </pre>
         * <p>
         * Note that the RecyclerView requires you to dispatch adapter updates immediately
         * when you change the data (you cannot defer {@code notify*} calls). The usage
         * above adheres to this rule because updates are sent to the adapter right after
         * the backing data is changed, before RecyclerView tries to read it.
         * <p>
         * On the other hand, if you have another {@link RecyclerView.AdapterDataObserver
         * AdapterDataObserver} that tries to process events synchronously, this may
         * confuse that observer because the list is instantly moved to its final state
         * while the adapter updates are dispatched later on, one by one. If you have such
         * an {@link RecyclerView.AdapterDataObserver
         * AdapterDataObserver}, you can use {@link #dispatchUpdatesTo(ListUpdateCallback)}
         * to handle each modification manually.
         * @param adapter A RecyclerView adapter which was displaying the old list and
         * will start displaying the new list.
         */
         ...
         * <p>
         * These updates are atomic such that the first update call effects every update
         * call that comes after it (the same as RecyclerView).
            ...
            for (int i = count - 1; i >= 0; i--) {
                ...
                switch (status) {
                    ...
                    case FLAG_IGNORE: // ignoring this
                        ...
                        break;
                    default:
                        throw new IllegalStateException("unknown flag for pos " + (globalIndex + i) + " " + Long
     ...
     * <p>
     * When an update is skipped, it is tracked as other updates are dispatched until the
     * matching add/remove operation is found at which point the tracked position is used
     * to dispatch the update.
     */
