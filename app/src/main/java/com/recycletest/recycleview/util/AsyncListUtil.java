 ...
 * <p>
 * It can be used to load Cursor data in chunks without querying the Cursor on the UI
 * Thread while keeping UI and cache synchronous for better user experience.
 * <p>
 ...
 * loads the required data items in the background through {@link DataCallback}, and
 * notifies a {@link ViewCallback} when the data is loaded. It may load some extra items
 * for smoother scrolling.
 * <p>
 * Note that this class uses a single thread to load the data, so it suitable to load data
 * from secondary storage such as disk, but not from network.
 * <p>
 * This class is designed to work with {@link RecyclerView}, but
 * it does not depend on it and can be used with other list views.
 */
public class AsyncListUtil<T> {
     ...
     * @param viewCallback Callback for querying visible item range and update
     * notifications.
     */
     ...
     * <p>
     * <p>
     * Identifies the data items that have not been loaded yet and initiates loading them
     * in the background. Should be called from the view's scroll listener (such as {@link
     ...
     * <p>
     * Discards all the cached data and reloads all required data items for the currently
     * visible range. To be called when the data item count and/or contents has changed.
     */
    public void refresh() {
    ...
    /**
     * Returns the data item at the given position or <code>null</code> if it has not been
     * loaded yet.
     * <p>
     * <p>
     * If this method has been called for a specific position and returned
     * <code>null</code>, then {@link ViewCallback#onItemLoaded(int)} will be called when
     * it finally loads. Note that if this position stays outside of the cached item range
     * (as defined by {@link ViewCallback#extendRangeInto} method), then the callback will
     * never be called for this position.
     * @param position Item position.
     * @return The data item at the given position or <code>null</code> if it has not been
     * loaded yet.
     */
    ...
    private final ThreadUtil.MainThreadCallback<T> mMainThreadCallback = new ThreadUtil.MainThreadCallback<T>() {
        ...
        public void addTile(int generation, TileList.Tile<T> tile) {
            ...
            if (DEBUG) {
                log("gen #%d, added tile @%d, total tiles: %d", generation, tile.mStartPosition, mTileList
                        .size());
            }
    ...
    public static abstract class DataCallback<T> {
         ...
         * <p>
         * <p>
         * If the data is being accessed through {@link android.database.Cursor} this is
         * where the new cursor should be created.
         ...
         * <p>
         * <p>
         * The provided tile might be a recycled tile, in which case it will already have
         * objects. It is suggested to re-use these objects if possible in your use case.
         ...
         * @param data Array of data items. Should not be accessed beyond
         * <code>itemCount</code>.
         * @param itemCount The data item count.
         */
        @WorkerThread
         ...
         * <p>
         * <p>
         * The actual number of cached tiles will be the maximum of this value and the
         * number of tiles that is required to cover the range returned by {@link
         * ViewCallback#extendRangeInto(int[], int[], int)}.
         * <p>
         * For example, if this method returns 10, and the most recent call to {@link
         * ViewCallback#extendRangeInto(int[], int[], int)} returned {100, 179}, and the
         * tile size is 5, then the maximum number of cached tiles will be 16.
         * <p>
         * However, if the tile size is 20, then the maximum number of cached tiles will
         * be 10.
         * <p>
    ...
    public static abstract class ViewCallback {
        ...
        /**
         * Scrolling in descending order (from higher to lower positions in the order of
         * the backing storage).
         */
        public static final int HINT_SCROLL_DESC = 1;

        /**
         * Scrolling in ascending order (from lower to higher positions in the order of
         * the backing storage).
         */
         ...
         * <p>
         * outRange[0] is the position of the first visible item (in the order of the
         * backing storage).
         * <p>
         * outRange[1] is the position of the last visible item (in the order of the
         * backing storage).
         * <p>
         * Negative positions and positions greater or equal to {@link #getItemCount} are
         * invalid. If the returned range contains invalid positions it is ignored (no
         * item will be loaded).
         ...
         * <p>
         * <p>
         * If there is no scroll hint, the default implementation extends the visible
         * range by half its length in both directions. If there is a scroll hint, the
         * range is extended by its full length in the scroll direction, and by half in
         * the other direction.
         * <p>
         * For example, if <code>range</code> is <code>{100, 200}</code> and
         * <code>scrollHint</code> is {@link #HINT_SCROLL_ASC}, then <code>outRange</code>
         * will be <code>{50, 300}</code>.
         * <p>
