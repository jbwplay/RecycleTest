 ...
 * gaps, StaggeredGridLayoutManager can offset spans independently or move items between
 * spans. You can control this behavior via {@link #setGapStrategy(int)}.
 */
public class StaggeredGridLayoutManager extends RecyclerView.LayoutManager implements RecyclerView.SmoothScroller.ScrollVectorProvider {
    ...
    /**
     * When scroll state is changed to {@link RecyclerView#SCROLL_STATE_IDLE},
     * StaggeredGrid will check if there are gaps in the because of full span items. If it
     * finds, it will re-layout and move items to correct positions with animations.
     * <p>
     * For example, if LayoutManager ends up with the following layout due to adapter
     * changes:
     * <pre>
     * AAA
     * _BC
    ...
    /**
     * Primary orientation is the layout's orientation, secondary orientation is the
     * orientation for spans. Having both makes code much cleaner for calculations.
     */
    @NonNull
    ...
    /**
     * Keeps the mapping between the adapter positions and spans. This is necessary to
     * provide a consistent experience when user scrolls the list.
     */
    ...
    /**
     * SavedState is not handled until a layout happens. This is where we keep it until
     * next layout.
     */
    ...
    /**
     * If a full span item is invalid / or created in reverse direction; it may create
     * gaps in the UI. While laying out, if such case is detected, we set this flag.
     * <p>
    ...
    /**
     * Temporary array used (solely in {@link #collectAdjacentPrefetchPositions}) for
     * stashing and sorting distances to views being prefetched.
     */
     ...
     * <p>
     * When a full span item is laid out in reverse direction, it sets a flag which we
     * check when scroll is stopped (or re-layout happens) and re-layout after first valid
     * item.
     */
     ...
     * <p>
     * Intermediate gaps created by full span items are tracked via
     * mLaidOutInvalidFullSpan field.
     */
    View hasGapsToFix() {
    ...
    /**
     * Sets the number of spans for the layout. This will invalidate all of the span
     * assignments for Views.
     * <p>
     * Calling this method will automatically result in a new layout request unless the
     * spanCount parameter is equal to current span count.
    ...
    /**
     * Sets the orientation of the layout. StaggeredGridLayoutManager will do its best to
     * keep scroll position if this method is called after views are laid out.
     ...
     * The order items are traversed is not affected by this call.
     * <p>
     * For vertical layout, if it is set to <code>true</code>, first item will be at the
     * bottom of the list.
     * <p>
     ...
     * <p>
     * Staggered grid may have gaps in the layout due to changes in the adapter. To avoid
     * gaps, StaggeredGridLayoutManager provides 2 options. Check {@link
     * #GAP_HANDLING_NONE} and {@link #GAP_HANDLING_MOVE_ITEMS_BETWEEN_SPANS} for
     * details.
     * <p>
    ...
    /**
     * Sets the gap handling strategy for StaggeredGridLayoutManager. If the gapStrategy
     * parameter is different than the current strategy, calling this method will trigger
     * a layout request.
     * @param gapStrategy The new gap handling strategy. Should be {@link
     * #GAP_HANDLING_MOVE_ITEMS_BETWEEN_SPANS} or {@link #GAP_HANDLING_NONE}.
     * @see #getGapStrategy()
     */
    ...
    /**
     * For consistency, StaggeredGridLayoutManager keeps a mapping between spans and
     * items.
     * <p>
     * If you need to cancel current assignments, you can call this method which will
     * clear all assignments and request a new layout.
     */
    ...
    /**
     * Calculates the views' layout order. (e.g. from end to start or start to end) RTL
    ...
    private void onLayoutChildren(RecyclerView.Recycler recycler, RecyclerView.State state, boolean shouldCheckForGaps) {
        ...
        boolean recalculateAnchor = !anchorInfo.mValid || mPendingScrollPosition != RecyclerView.NO_POSITION || mPendingSavedState != null;
    ...
    private boolean updateAnchorFromChildren(RecyclerView.State state, AnchorInfo anchorInfo) {
        ...
        anchorInfo.mPosition = mLastLayoutFromEnd ? findLastReferenceChildPosition(state.getItemCount()) : findFirstReferenceChildPosition(state
                .getItemCount());
        ...
        if (mPendingSavedState == null || mPendingSavedState.mAnchorPosition == RecyclerView.NO_POSITION || mPendingSavedState.mSpanOffsetsSize < 1) {
            ...
            if (child != null) {
                ...
                if (childSize > mPrimaryOrientation.getTotalSpace()) {
                    ...
                    anchorInfo.mOffset = anchorInfo.mLayoutFromEnd ? mPrimaryOrientation.getEndAfterPadding() : mPrimaryOrientation
                            .getStartAfterPadding();
                    return true;
                }

                final int startGap = mPrimaryOrientation.getDecoratedStart(child) - mPrimaryOrientation
                        .getStartAfterPadding();
                if (startGap < 0) {
                    anchorInfo.mOffset = -startGap;
                    return true;
                }
                final int endGap = mPrimaryOrientation.getEndAfterPadding() - mPrimaryOrientation
    ...
    void updateMeasureSpecs(int totalSpace) {
        ...
        mFullSizeSpec = View.MeasureSpec.makeMeasureSpec(totalSpace, mSecondaryOrientation
                .getMode());
    }
     ...
     * well.
     * <p>
     * StaggeredGridLayoutManager may pre-cache some views that are not necessarily
     * visible. Those views are ignored in this method.
     * @param into An array to put the results into. If you don't provide any,
     * LayoutManager will create a new one.
     * @return The adapter position of the first visible item in each span. If a span does
     ...
     * @param into An array to put the results into. If you don't provide any,
     * LayoutManager will create a new one.
     * @return The adapter position of the first fully visible item in each span. If a
     * span does not have any items, {@link RecyclerView#NO_POSITION} is returned for that
     * span.
     ...
     * @param into An array to put the results into. If you don't provide any,
     * LayoutManager will create a new one.
     * @return The adapter position of the last fully visible item in each span. If a span
     * does not have any items, {@link RecyclerView#NO_POSITION} is returned for that
     * span.
     * @see #findFirstCompletelyVisibleItemPositions(int[])
     * @see #findLastVisibleItemPositions(int[])
     */
    ...
    private void measureChildWithDecorationsAndMargin(View child, LayoutParams lp, boolean alreadyMeasured) {
        if (lp.mFullSpan) {
            if (mOrientation == VERTICAL) {
                measureChildWithDecorationsAndMargin(child, mFullSizeSpec, getChildMeasureSpec(getHeight(), getHeightMode(), 0, lp.height, true), alreadyMeasured);
            } else {
                measureChildWithDecorationsAndMargin(child, getChildMeasureSpec(getWidth(), getWidthMode(), 0, lp.width, true), mFullSizeSpec, alreadyMeasured);
            }
        } else {
            if (mOrientation == VERTICAL) {
                measureChildWithDecorationsAndMargin(child, getChildMeasureSpec(mSizePerSpan, getWidthMode(), 0, lp.width, false), getChildMeasureSpec(getHeight(), getHeightMode(), 0, lp.height, true), alreadyMeasured);
            } else {
                measureChildWithDecorationsAndMargin(child, getChildMeasureSpec(getWidth(), getWidthMode(), 0, lp.width, true), getChildMeasureSpec(mSizePerSpan, getHeightMode(), 0, lp.height, false), alreadyMeasured);
            }
        }
    }
    ...
    public void onInitializeAccessibilityNodeInfoForItem(RecyclerView.Recycler recycler, RecyclerView.State state, View host, AccessibilityNodeInfoCompat info) {
        ...
        if (mOrientation == HORIZONTAL) {
            ...
            info.setCollectionItemInfo(AccessibilityNodeInfoCompat.CollectionItemInfoCompat
                    .obtain(-1, -1, sglp.getSpanIndex(), sglp.mFullSpan ? mSpanCount : 1, sglp.mFullSpan, false));
        }
    }
    ...
    /**
     * Finds the first fully visible child to be used as an anchor child if span count
     * changes when state is restored. If no children is fully visible, returns a
     * partially visible child instead of returning null.
     */
    ...
    /**
     * This is for internal use. Not necessarily the child closest to start but the first
     * child we find that matches the criteria. This method does not do any sorting based
     * on child's start coordinate, instead, it uses children order.
     */
    ...
    /**
     * This is for internal use. Not necessarily the child closest to bottom but the first
     * child we find that matches the criteria. This method does not do any sorting based
     * on child's end coordinate, instead, it uses children order.
     */
    ...
    private void updateLayoutState(int anchorPosition, RecyclerView.State state) {
        ...
        mLayoutState.mInfinite = mPrimaryOrientation.getMode() == View.MeasureSpec.UNSPECIFIED && mPrimaryOrientation
                .getEnd() == 0;
    }
    ...
    /**
     * Checks whether it should invalidate span assignments in response to an adapter
     * change.
     */
    ...
    private int fill(RecyclerView.Recycler recycler, LayoutState layoutState, RecyclerView.State state) {
        ...
        final int defaultNewViewLine = mShouldReverseLayout ? mPrimaryOrientation.getEndAfterPadding() : mPrimaryOrientation
                .getStartAfterPadding();
        boolean added = false;
        while (layoutState.hasMore(state) && (mLayoutState.mInfinite || !mRemainingSpans.isEmpty())) {
            ...
            final int otherEnd;
            if (isLayoutRTL() && mOrientation == VERTICAL) {
                otherEnd = lp.mFullSpan ? mSecondaryOrientation.getEndAfterPadding() : mSecondaryOrientation
                        .getEndAfterPadding() - (mSpanCount - 1 - currentSpan.mIndex) * mSizePerSpan;
                otherStart = otherEnd - mSecondaryOrientation.getDecoratedMeasurement(view);
            } else {
                otherStart = lp.mFullSpan ? mSecondaryOrientation.getStartAfterPadding() : currentSpan.mIndex * mSizePerSpan + mSecondaryOrientation
                        .getStartAfterPadding();
    ...
    private void recycleFromEnd(RecyclerView.Recycler recycler, int line) {
        ...
        for (i = childCount - 1; i >= 0; i--) {
            View child = getChildAt(i);
            if (mPrimaryOrientation.getDecoratedStart(child) >= line && mPrimaryOrientation
                    .getTransformedStartWithDecoration(child) >= line) {
                LayoutParams lp = (LayoutParams) child.getLayoutParams();
                // Don't recycle the last View in a span not to lose span's start/end lines
                if (lp.mFullSpan) {
                    for (int j = 0; j < mSpanCount; j++) {
    ...
    public void collectAdjacentPrefetchPositions(int dx, int dy, RecyclerView.State state, LayoutPrefetchRegistry layoutPrefetchRegistry) {
        ...
        for (int i = 0; i < mSpanCount; i++) {
            ...
            int distance = mLayoutState.mItemDirection == LayoutState.LAYOUT_START ? mLayoutState.mStartLine - mSpans[i]
                    .getStartLine(mLayoutState.mStartLine) : mSpans[i].getEndLine(mLayoutState.mEndLine) - mLayoutState.mEndLine;
            if (distance >= 0) {
    ...
    public View onFocusSearchFailed(View focused, int direction, RecyclerView.Recycler recycler, RecyclerView.State state) {
        ...
        if (!prevFocusFullSpan) {
            unfocusableCandidate = findViewByPosition(shouldSearchFromStart ? prevFocusSpan
        ...
        if (preferLastSpan(layoutDir)) {
            for (int i = mSpanCount - 1; i >= 0; i--) {
                ...
                unfocusableCandidate = findViewByPosition(shouldSearchFromStart ? mSpans[i]
                        .findFirstPartiallyVisibleItemPosition() : mSpans[i].findLastPartiallyVisibleItemPosition());
                if (unfocusableCandidate != null && unfocusableCandidate != directChild) {
                    return unfocusableCandidate;
                }
            }
     ...
     * <p>
     * Note that if the orientation is {@link #VERTICAL}, the width parameter is ignored
     * and if the orientation is {@link #HORIZONTAL} the height parameter is ignored
    ...
    public static class LayoutParams extends RecyclerView.LayoutParams {
        ...
        /**
         * When set to true, the item will layout using all span area. That means, if
         * orientation is vertical, the view will have full width; if orientation is
         * horizontal, the view will have full height.
         ...
         * @return The Span index of the View. If View is not yet assigned to any span,
         * returns {@link #INVALID_SPAN_ID}.
         */
    ...
    class Span {
        ...
        void cacheReferenceLineAndClear(boolean reverseLayout, int offset) {
            ...
            if ((reverseLayout && reference < mPrimaryOrientation.getEndAfterPadding()) || (!reverseLayout && reference > mPrimaryOrientation
                    .getStartAfterPadding())) {
                return;
            }
        ...
        public int findFirstVisibleItemPosition() {
            return mReverseLayout ? findOneVisibleChild(mViews.size() - 1, -1, false) : findOneVisibleChild(0, mViews
                    .size(), false);
        }

        public int findFirstPartiallyVisibleItemPosition() {
            return mReverseLayout ? findOnePartiallyVisibleChild(mViews.size() - 1, -1, true) : findOnePartiallyVisibleChild(0, mViews
                    .size(), true);
        }

        public int findFirstCompletelyVisibleItemPosition() {
            return mReverseLayout ? findOneVisibleChild(mViews.size() - 1, -1, true) : findOneVisibleChild(0, mViews
                    .size(), true);
        }

        public int findLastVisibleItemPosition() {
            return mReverseLayout ? findOneVisibleChild(0, mViews.size(), false) : findOneVisibleChild(mViews
                    .size() - 1, -1, false);
        }

        public int findLastPartiallyVisibleItemPosition() {
            return mReverseLayout ? findOnePartiallyVisibleChild(0, mViews.size(), true) : findOnePartiallyVisibleChild(mViews
                    .size() - 1, -1, true);
        }

        public int findLastCompletelyVisibleItemPosition() {
            return mReverseLayout ? findOneVisibleChild(0, mViews.size(), true) : findOneVisibleChild(mViews
                    .size() - 1, -1, true);
        }

        /**
         * Returns the first view within this span that is partially or fully visible.
         * Partially visible refers to a view that overlaps but is not fully contained
         * within RV's padded bounded area. This view returned can be defined to have an
         * area of overlap strictly greater than zero if acceptEndPointInclusion is false.
         * If true, the view's endpoint inclusion is enough to consider it partially
         * visible. The latter case can then refer to an out-of-bounds view positioned
         * right at the top (or bottom) boundaries of RV's padded area. This is used e.g.
         * inside {@link #onFocusSearchFailed(View, int, RecyclerView.Recycler, *
         * RecyclerView.State)} for calculating the next unfocusable child to become
         ...
         * @param completelyVisible True if we have to only consider completely visible
         * views, false otherwise.
         * @param acceptCompletelyVisible True if we can consider both partially or fully
         * visible views, false, if only a partially visible child should be returned.
         * @param acceptEndPointInclusion If the view's endpoint intersection with RV's
         * padded bounded area is enough to consider it partially visible, false
         * otherwise
         * @return The adapter position of the first view that's either partially or fully
         * visible. {@link RecyclerView#NO_POSITION} if no such view is found.
         */
        ...
        /**
         * Depending on the layout direction, returns the View that is after the given
         * position.
         */
    ...
    static class LazySpanLookup {
         ...
         * @param hasUnwantedGapAfter If true, when full span item has unwanted gaps, it
         * will be returned even if its gap direction does not match.
         */
        ...
        /**
         * We keep information about full span items because they may create gaps in the
         * UI.
         */
        static class FullSpanItem implements Parcelable {
            ...
            public String toString() {
                return "FullSpanItem{" + "mPosition=" + mPosition + ", mGapDir=" + mGapDir + ", mHasUnwantedGapAfter=" + mHasUnwantedGapAfter + ", mGapPerSpan=" + Arrays
    ...
    /**
     * Data class to hold the information about an anchor position which is used in
     * onLayout call.
     */
    class AnchorInfo {
        ...
        void assignCoordinateFromPadding() {
            mOffset = mLayoutFromEnd ? mPrimaryOrientation.getEndAfterPadding() : mPrimaryOrientation
                    .getStartAfterPadding();
        }
